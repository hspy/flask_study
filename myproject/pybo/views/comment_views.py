...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        return redirect('{}#comment_{}'.format(
            url_for('question.detail', question_id=question_id), comment.id)) #앵커 추가
    return render_template('comment/comment_form.html', form=form)

@bp.route('/modify/question/<int:comment_id>', methods=('GET', 'POST'))
@login_required
def modify_question(comment_id):
    comment = Comment.query.get_or_404(comment_id) #해당 커멘트 아이디 존재하는지, 없으면 404
    if g.user != comment.user:  #해당 유저가 아니면
        flash('수정권한이 없습니다')
        return redirect(url_for('question.detail', question_id=comment.question.id))
    if request.method == 'POST': #Used to send HTML form data to the server. The data received by the POST method is not cached by the server.
        form = CommentForm()
        if form.validate_on_submit(): #입력 가능하면
            form.populate_obj(comment) #커멘트 내용
            comment.modify_date = datetime.now()  # 수정일시 저장
            db.session.commit() #디비에 추가
            return redirect('{}#comment_{}'.format(
                url_for('question.detail', question_id=comment.question.id), comment.id)) #앵커 추가
    else:
